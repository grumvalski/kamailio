...
extern str ecscf_record_route_mt_uri;
...
void alter_dialog_route_set(dlg_t *,enum e_dialog_direction);
int send_request(str ,str ,dlg_t *,transaction_cb , enum e_dialog_direction);
 ...
 */
int release_call_confirmed(e_dialog *d, int reason_code, str reason_text)
{
	enum e_dialog_direction odir;
	e_dialog *o;
	str r;
	str hdrs={0,0};	
	char buf[256];
	...
	o = get_e_dialog_dir_nolock(d->call_id,odir);
...
{		
	...
	if (!ps->param) return;
	dir = *((enum e_dialog_direction *) *(ps->param));
	shm_free(*ps->param);
	*ps->param = 0;		
 ...
 */
int release_call_previous(e_dialog *d,enum release_call_situation situation,int reason_code,str reason_text)
{
	struct cell* t;
	e_dialog *o;
	enum e_dialog_direction odir;
...
int release_call_early(e_dialog *d,int reason_code,str reason_text)
{
...
int release_call_early200(e_dialog *d,int reason_code,str reason_text)
{
...
int release_call(str callid,int reason_code,str reason_text)
{
	...
	d = get_e_dialog_dir(callid,DLG_MOBILE_ORIGINATING);
	if (d) {				
		hash = d->hash;
		if (release_call_e(d,reason_code,reason_text)>0) res = 1;		
		goto done;		
	}
	d = get_e_dialog_dir(callid,DLG_MOBILE_TERMINATING);
	if (d) {				
		hash = d->hash;
		if (release_call_e(d,reason_code,reason_text)>0) res = 1;		
		goto done;
...
/*int E_release_call_onreply(struct sip_msg *msg,char *str1,char *str2)
{
	...
	if (is_e_dialog_dir(callid,dir)) {
		d=get_e_dialog_dir(callid,dir);
 ...
 */
void alter_dialog_route_set(dlg_t *d,enum e_dialog_direction dir)
{
	rr_t *r,*r_new;	
	...
	switch (dir) {
		case DLG_MOBILE_ORIGINATING:
			p = ecscf_record_route_mo_uri;
			break;
		case DLG_MOBILE_TERMINATING:
			p = ecscf_record_route_mt_uri;
			break;
		default:
			return;
	}
