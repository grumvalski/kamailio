...
int LRF_subscribe_LOCSIP(struct sip_msg *req, char* str1, char* str2)
{
	...
	LM_DBG("trying to send a subscribe for public id %.*s\n",
...
int LRF_process_notification(struct sip_msg* msg, char * str1, char* str2){
	...
	LM_DBG("Checking NOTIFY\n");

	if(!cscf_get_from_uri(msg, &user_uri) || !user_uri.len || !user_uri.s){
		LM_ERR("could not retrieve the From uri\n");
		goto parse_error;
	}
	...
	if(!subscr){
		...
		LM_ERR("could not retrieve the location subscription for uri %.*s\n", user_uri.len, user_uri.s);
		goto subscr_error;		
	}
	...
	if(!loc){
		LM_ERR("could not retrieve the PIDF-LO\n");
		goto parse_error;
	}
reply:
	if((ret = sl_reply(msg, (char *)&fp_200, (char *)&fp_ok)) == -1){
		LM_ERR("could not send the 200 ok reply\n");
		goto internal_error;
	}
	
	//release the notify transaction
	if(tmb.t_release(msg) == -1){
		LM_ERR("could not release the notify transaction\n");
		goto internal_error;
	}
	...
	if(!user_data){
		LM_ERR("null user_data in the subscription info for uri %.*s\n", user_uri.len, user_uri.s);
		goto internal_error;		
	}
...
int LRF_sw2options(struct sip_msg* msg, char* str1, char* str2){
	...
	LM_DBG("trying to switch to the OPTIONS transaction\n");
	...
	if(sw_tr_2_options_tr(msg, user_data)<0){
		...
		LM_ERR("could not switch to the options trans\n");
		goto end;
	}
...
xmlNode * retrieve_pidf_lo(struct sip_msg * msg, str * pidf_body, loc_fmt * crt_loc_fmt){
	...
	if (content_type.len!=pidflo_info.len ||
		...
		LM_ERR("The content should be %.*s but it is %.*s\n",
