 ...
 */
static inline void lrf_dialog_count_lock()
{
 ...
 */
static inline void lrf_dialog_count_unlock()
{
 ...
 */
static inline int lrf_dialog_count_increment ()
{
	...
	LM_DBG("LRF Dialog counter value is %d\n", *lrf_dialog_count);
}

/**
 * Decrement the dialog count
 */
static inline void lrf_dialog_count_decrement()
{
...
lrf_dialog* new_lrf_dialog(str call_id, str target_uri)
{
	...
	if (!d) {
		LM_ERR("Unable to alloc %d bytes\n", (int)sizeof(lrf_dialog));
		goto error;
	}
	memset(d,0,sizeof(lrf_dialog));
	
	d->hash = get_lrf_dialog_hash(call_id, lrf_dialogs_hash_size);		
...
lrf_dialog* add_lrf_dialog(str call_id,str target_URI)
{
	...
	d_lock(lrf_dialogs[d->hash].lock);
		d->next = 0;
...
{
	lrf_dialog *d=0;
	unsigned int hash = get_lrf_dialog_hash(call_id, lrf_dialogs_hash_size);

	d_lock(lrf_dialogs[hash].lock);
		d = lrf_dialogs[hash].head;
		while(d){
			if ((!dir || d->direction == *dir) &&
				d->call_id.len == call_id.len &&
				strncasecmp(d->call_id.s,call_id.s,call_id.len)==0) {
					d_unlock(lrf_dialogs[hash].lock);
					return 1;
				}
			d = d->next;
		}
...
ok:
	if (d) d_unlock(lrf_dialogs[d->hash].lock);
	return CSCF_RETURN_TRUE;	
nok:
	if (d) d_unlock(lrf_dialogs[d->hash].lock);
	return CSCF_RETURN_FALSE;		
