...
loc_subscription* loc_subscribe(str uri,int duration, user_d * user_data)
{
	...
	if (s){
		...
		if (!s){
			LM_ERR("Error creating new subscription\n");
			return 0;
		}
...
int loc_send_subscribe(loc_subscription *s, str dest, int duration)
{
	...
	set_uac_req(&uac_r, &method, &h, 0, 0, TMCB_LOCAL_COMPLETED, loc_subscribe_response, (void*)s->user_data);
...
void subscription_err_resp(struct cell * t, struct tmcb_params *ps){
	...
	LM_DBG("user_data: %p, user_uri %.*s, OPTIONS trans index: %u label: %u callid: %.*s\n", 
	...
	if (tmb.t_lookup_ident(&options_trans, user_data->options_tr.hash_index, user_data->options_tr.label) < 0) {
		LM_ERR("could not switch to the OPTIONS transaction\n");
		goto error;
	}

	LM_DBG("OPTIONS trans %p callid: %.*s\n", options_trans, options_trans->callid.len, options_trans->callid.s);

	if(tmb.t_reply(options_trans->uas.request, 405, NO_LOCATION)<0){
		LM_ERR("Could not reply to the OPTIONS request\n");
		goto error;
	}
	LM_ERR("sent a 405 no available location error response\n");

	tmb.t_release(options_trans->uas.request);
	
	// switch to the original transaction
...
void loc_subscribe_response(struct cell *t,int type,struct tmcb_params *ps)
{
	...
	if(!cscf_get_to_uri(ps->rpl, &req_uri)){
		LM_ERR("could not retrieve the to uri\n");
		return;
	}

	str callid = cscf_get_call_id(ps->rpl, NULL);
	if(!callid.len || !callid.s){
		LM_ERR("could not retrieve the callid header\n");
		//TODO: error handling
		return;
	}
	...
	if (!s){
		LM_ERR("received a SUBSCRIBE response but no subscription for <%.*s>\n",
	...
	if (ps->code>=200 && ps->code<300){
		...
		LM_ERR("SUBSCRIRE error response code %d\n",ps->code);				
	...
	if(tmb.t_release(ps->rpl) == -1){
		LM_ERR("could not release the subscribe transaction\n");
	}else
		LM_ERR("released the subscribe transaction\n");
...
{
	LM_DBG("refreshing subscription for <%.*s> [%d]\n",
...
loc_subscription* get_loc_subscription_callid(str aor, str callid)
{
	...
	subs_lock(hash);
		s = subscriptions[hash].head;
		while(s){
			LM_DBG("crt callid is %.*s, compared to %.*s\n",
