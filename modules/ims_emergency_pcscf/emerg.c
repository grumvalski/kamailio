...
{
	LM_INFO("Check if the P-CSCF is configured to accept an anonymous emergency call or not\n");
...
int P_is_anonymous_user(struct sip_msg *msg,char *str1,char *str2)
{
	...
	LM_INFO("Check if anonymous identity used\n");
	...
	if((from_body->display.len == anonym_display.len) &&
		...
		LM_INFO("using anonymous identity\n");
		return CSCF_RETURN_TRUE;
	}
