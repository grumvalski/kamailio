...
error:
	LM_ERR("invalid emergency URN %.*s\n", len, uri);
	return NOT_EM_URN;

not_emerg:
	LM_ERR("no emergency URN %.*s\n", len, uri);
	return NOT_EM_URN;
...
urn_t check_emerg_number(str user, str* urn){
	...
	for(emerg_number = emerg_nb_list; emerg_number!=NULL; emerg_number = emerg_number->next){
		LM_DBG("comparing to %.*s\n", emerg_number->number.len, emerg_number->number.s);
...
int read_em_nb(xmlNode * node, str* number, str* urn){
	...
	if(!nb || !(content = xmlNodeGetContent((xmlNodePtr)nb)) || !(strlen((char*)content))){
		LM_ERR("invalid ot not found %s tag, invalid config file\n", NUMBER_NODE_NAME);
		return -1;
	}
	...
	if(!urn_node ||  !(content2 = xmlNodeGetContent((xmlNodePtr)urn_node))
		...
		LM_ERR("invalid or not found %s tag, invalid config file\n", URN_NODE_NAME);
		xmlFree(content);
		return -1;
	}
...
int store_em_numbers(){
	...
	if(!emerg_numbers_file || emerg_numbers_file[0]=='\0')
		return 0;
	LM_DBG("loading numbers from file %s\n", emerg_numbers_file);
